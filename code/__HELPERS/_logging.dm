...
/proc/log_say(text, mob/speaker)
	if(config.log_say)
		WRITE_LOG(GLOB.world_game_log, "SAY: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_whisper(text, mob/speaker)
	if(config.log_whisper)
		WRITE_LOG(GLOB.world_game_log, "WHISPER: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_ooc(text, client/user)
	if(config.log_ooc)
		WRITE_LOG(GLOB.world_game_log, "OOC: [user.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_aooc(text, client/user)
	if(config.log_ooc)
		WRITE_LOG(GLOB.world_game_log, "AOOC: [user.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_looc(text, client/user)
	if(config.log_ooc)
		WRITE_LOG(GLOB.world_game_log, "LOOC: [user.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_emote(text, mob/speaker)
	if(config.log_emote)
		WRITE_LOG(GLOB.world_game_log, "EMOTE: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")
...
/proc/log_adminsay(text, mob/speaker)
	if(config.log_adminchat)
		WRITE_LOG(GLOB.world_game_log, "ADMINSAY: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_mentorsay(text, mob/speaker)
	if(config.log_adminchat)
		WRITE_LOG(GLOB.world_game_log, "MENTORSAY: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_ghostsay(text, mob/speaker)
	if(config.log_say)
		WRITE_LOG(GLOB.world_game_log, "DEADCHAT: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_ghostemote(text, mob/speaker)
	if(config.log_emote)
		WRITE_LOG(GLOB.world_game_log, "DEADEMOTE: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_adminwarn(text)
	if(config.log_adminwarn)
		WRITE_LOG(GLOB.world_game_log, "ADMINWARN: [lhtml_decode(text)][log_end]")

/proc/log_pda(text, mob/speaker)
	if(config.log_pda)
		WRITE_LOG(GLOB.world_game_log, "PDA: [speaker.simple_info_line()]: [lhtml_decode(text)][log_end]")

/proc/log_chat(text, mob/speaker)
	if(config.log_pda)
		WRITE_LOG(GLOB.world_game_log, "CHAT: [speaker.simple_info_line()] [lhtml_decode(text)][log_end]")
...
/proc/log_world(text)
	...
	if(config && config.log_world_output)
		WRITE_LOG(GLOB.world_game_log, "WORLD: [lhtml_decode(text)][log_end]")
...
/proc/log_href(text)
	WRITE_LOG(GLOB.world_href_log, "HREF: [lhtml_decode(text)]")
