...
/datum/data/pda/app/supply/update_ui(mob/user as mob, list/data)
	...
	for(var/S in shuttle_master.shoppinglist)
		...
		supplyOrderCount++
		supplyOrderData[++supplyOrderData.len] = list("Number" = SO.ordernum, "Name" = lhtml_decode(SO.object.name), "ApprovedBy" = SO.orderedby, "Comment" = lhtml_decode(SO.comment))
	...
	for(var/S in shuttle_master.requestlist)
		var/datum/supply_order/SO = S
		requestCount++
		requestData[++requestData.len] = list("Number" = SO.ordernum, "Name" = lhtml_decode(SO.object.name), "OrderedBy" = SO.orderedby, "Comment" = lhtml_decode(SO.comment))
