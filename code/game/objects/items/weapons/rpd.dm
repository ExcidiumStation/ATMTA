...
/obj/item/rpd/Topic(href, href_list, nowindow, state)
	...
	if(href_list["iconrotation"])
		iconrotation = text2num(sanitize_local(href_list["iconrotation"]))
	else if(href_list["whatpipe"])
		whatpipe = text2num(sanitize_local(href_list["whatpipe"]))
	else if(href_list["whatdpipe"])
		whatdpipe = text2num(sanitize_local(href_list["whatdpipe"]))
	else if(href_list["pipetype"])
		pipetype = text2num(sanitize_local(href_list["pipetype"]))
	else if(href_list["mode"])
		mode = text2num(sanitize_local(href_list["mode"]))
	else
		return
